...
export default function CollectionPage() {
  ...
  async function handleAdd(e) {
    e.preventDefault()
    setFormError('')
    if (!form.name || !form.address || !form.tel || !form.amount) {
                ...
                {donors.map((d, i) => (
                  <tr key={d.sn} style={i % 2 === 0 ? s.trEven : s.trOdd}>
                    ...
                    <td style={{...s.td, color: '#c9a84c', fontWeight: 600, fontFamily: "'Playfair Display',serif"}}>
                      {parseFloat(d.amount).toLocaleString('en-LK', { minimumFractionDigits: 2 })}
                    </td>
                  </tr>
                ))}
              </tbody>
...
const s = {
  ...
  closeBtn: {
    background: 'none', border: 'none', color: '#8a9bb0', fontSize: 18, cursor: 'pointer',
  },
  ...
  cancelBtn: {
    ...
    color: '#8a9bb0', borderRadius: 8, padding: '12px', fontSize: 14, cursor: 'pointer',
  },
  submitBtn: {
    flex: 1,
    background: 'linear-gradient(135deg,#c9a84c,#e4c97e)',
